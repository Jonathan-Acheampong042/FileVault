...
export default function MagicLinkForm({ onBack }: Props) {
  ...
  async function handleSend() {
    ...
    setMsg({ text: '✉️ Magic link sent! Check your inbox and spam folder. The link expires in 1 hour.', type: 'success' })
  }

  return (
    <div>
      <div className="mb-5 text-center">
        <div className="mx-auto mb-3 flex h-[52px] w-[52px] items-center justify-center rounded-2xl border border-violet-400/30 bg-violet-400/15">
          <span className="material-symbols-outlined text-[28px] text-violet-400">magic_button</span>
        </div>
        <h3 className="mb-1 text-base font-bold text-white">Magic Link Sign-In</h3>
        <p className="text-[13px] text-slate-500">Enter your email and we'll send you a one-click sign-in link — no password needed.</p>
      </div>
      ...
      <button onClick={onBack} className="mt-3.5 w-full text-center text-xs text-slate-500 hover:text-slate-400">
        ← Back to sign in
      </button>
    </div>
  )
